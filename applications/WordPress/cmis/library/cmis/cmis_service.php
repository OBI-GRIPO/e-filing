...
<?php
  ...
  function postEntry($url, $properties = array (), $content = null, $content_type = "application/octet-stream", $options = array ()) {
    ...
    //DEBUG
    $this->dbg("DEBUG: postEntry: myURL = " . $myURL."\n\n\n\n");
    static $entry_template;
    if (!isset ($entry_template)) {
      $entry_template = CMISService :: getEntryTemplate();
    }
    $this->dbg("DEBUG: postEntry: entry_template = " . $entry_template."\n\n\n\n");
    $properties_xml = $this->processPropertyTemplates($objType, $properties);
    $this->dbg("DEBUG: postEntry: properties_xml = " . $properties_xml."\n\n\n\n");
    if (is_array($options)) {
      $hash_values = $options;
    } else {
      $hash_values = array ();
    ...
    $this->dbg("DEBUG: postEntry: hash_values = " . print_r($hash_values,true)."\n\n\n\n");
    $post_value = CMISRepositoryWrapper :: processTemplate($entry_template, $hash_values);
    $this->dbg("DEBUG: postEntry: post_value = " . $post_value."\n\n\n\n");
    $ret = $this->doPost($myURL, $post_value, MIME_ATOM_XML_ENTRY);
    $obj = $this->extractObject($ret->body);
    $this->cacheObjectInfo($obj);
    return $obj;
  ...
  function updateProperties($objectId, $properties = array (), $options = array ()) {
    ...
    $obj_url = $this->getLink($objectId, LINK_EDIT);
  ...
  function checkOut($objectId,$options = array()) {
    ...
    //ETHACK
    //$obj = $this->extractObject($ret->body);//ret does not have body
   // $this->cacheObjectInfo($obj);
    return array("objectID"=>$ret->id,"contentCopied"=>true);
  }
  ...
  function checkIn($objectId,$options = array()) {  
    //Dirty..make it beter!! na vro apo pou na perno to uri gia to service
    $myURL = substr ($this->workspace->collections[COLLECTION_CHECKED_OUT],0,-11);
    $myURL .="/pwc/i/".substr($objectId,strlen("workspace://SpacesStore/"));
    $myURL= CMISRepositoryWrapper :: getOpUrl($myURL, $options);
    $emptyEntry ='<?xml version="1.0" encoding="utf-8"?><entry xmlns="http://www.w3.org/2005/Atom"/>';
    $ret = $this->doPut($myURL,$emptyEntry, MIME_ATOM_XML_ENTRY);//Prosoxh sto type=entry!!! xoris auto den leitourgei
    $obj = $this->extractObject($ret->body);
    $this->cacheObjectInfo($obj);
    return $obj;
  }
  ...
  function cancelCheckOut($objectId,$options = array()) {
    ...
    $varmap = $options;
    $varmap["id"] = $objectId;
    $via = $this->getLink($objectId,LINK_VIA);
      
       $this->dbg("DEBUG: cancelCheckOut VIA=".$via);
    if (!$via) {
